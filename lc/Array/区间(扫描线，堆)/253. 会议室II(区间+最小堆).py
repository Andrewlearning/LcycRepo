...
class Solution(object):
    def minMeetingRooms(self, intervals):
        ...
        :rtype: int
        """
        # 使用sort(key = lambda x:x[0])是只排序第一个元素，第二个元素不管
        # 使用sort(), 会按第一个元素，第二个元素这样，从小往大排
        # 假如我们对待一个会议室，认为在同一时间的进出，只算作一间的话，那么[30,-1],[30,1]这样可以算作是一间
        # 假如我们对待一个会议室，认为在同一时间的进出，算作两间的话，那么得按照[30,1],[30,-1]这样排序计算
